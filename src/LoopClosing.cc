...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  if (mpLastMap->GetAllKeyFrames().size() < 12) {
    // cout << "LoopClousure: Stereo KF inserted without check, map is small: " << mpCurrentKF->mnId
    // << endl;
    mpKeyFrameDB->add(mpCurrentKF);
    mpCurrentKF->SetErase();
    return false;
  ...
  if (mnMergeNumCoincidences > 0) {
    ...
    int numProjMatches = 0;
    vector<MapPoint*> vpMatchedMPs;
    bool bCommonRegion = DetectAndReffineSim3FromLastKF(mpCurrentKF, mpMergeMatchedKF, gScw,
                                                        numProjMatches, mvpMergeMPs, vpMatchedMPs);
    if (bCommonRegion) {
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF,
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    // Verbose::PrintMess("Sim3 reffine: There are " + to_string(numOptMatches) + " matches after of
    // the optimization ", Verbose::VERBOSITY_DEBUG);
  ...
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      ...
      if (bConverge) {
        ...
        // cout <<"BoW: " << numProjMatches << " matches between " << vpMapPoints.size() << " points
        // with coarse Sim3" << endl;
  ...
  // We need to propagate the correction through the spanning tree
  {
    ...
    if (!mbStopGBA) {
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        << " connected KFs" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(80).size() <<
        " connected KF with 80 common matches or more" << endl; cout << "--It has " <<
        pKF->GetCovisiblesByWeight(50).size() << " connected KF with 50 common matches or more" <<
        endl; cout << "--It has " << pKF->GetCovisiblesByWeight(20).size() << " connected KF with 20
        ...
        cv::Scalar(255, 0, 0));
            ...
            string namefile = "./test_GBA/GBA_" + to_string(nLoopKF) + "_KF" + to_string(pKF->mnId)
        +"_D" + to_string(dist) +".png"; cv::imwrite(namefile, imLeft);
        }*/
